...
CLNet::CLNet(
    ...
    : NetBase(net_def, ws) {
  ws_ = ws;
  VLOG(1) << "Constructing CLNet " << net_def->name();
  ...
  for (int idx = 0; idx < net_def->op_size(); ++idx) {
    ...
    if (operator_def.has_device_option() && operator_def.device_option().device_type() == PROTO_OPENCL) {
...
vector<float> CLNet::TEST_Benchmark(
  ...
  if (gpu_out_blob->IsType<OpenCLTensor<DataType>>()) {
    auto &g_ = gpu_out_blob->Get<OpenCLTensor<DataType>>();
    ...
    for (int i = 0; i < main_runs; ++i) {
      ...
      for (auto& op : operators_) {
        ...
        CAFFE_ENFORCE(
            ...
            op_type,
            ") has failed.");
        if (opengl_device_[idx] && op_type != "CopyFromCL" && op_type != "Reshape") {
