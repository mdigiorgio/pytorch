...
TEST(OPENCLOperatorTest, ConvTranspose) {
 ...
 PopulateCPUBlob(&ws, true, "cpu_X", {1, channel_in, spatial, spatial}, 1337);
 PopulateCPUBlob(&ws, true, "W", {channel_in, channel_out, kern, kern}, 1337);
 PopulateCPUBlob(&ws, false, "b", {channel_out}, 0);

#define ADD_CONV_ARGS                                                   \
 {                                                                      \
   ADD_ARG((*def), "kernel", i, kern);                                  \
   ADD_ARG((*def), "stride", i, 4);                                     \
   ADD_ARG((*def), "pad", i, 2);                                        \
   ADD_ARG((*def), "order", s, "NCHW");                                 \
 }
  ...
  NetDef cpu_net;
  {
    OperatorDef* def = AddOp(&cpu_net, "ConvTranspose", {"cpu_X", "W", "b"}, {"ref_Y"});
    def->set_name("cpu_conv_transpose");
    ADD_CONV_ARGS;
  }
  ...
  gpu_net.set_type("opencl");
  {
    ...
    OperatorDef* def = AddOp(&gpu_net, "ConvTranspose", {"cpu_X", "W", "b"}, {"gpu_Y"});
    MAKE_OPENCL_OPERATOR(def);
    ADD_CONV_ARGS;
  }
